...
namespace projet_Fulbank.Class.Model
{
    public class UserManager
    {
        ...
        public static User getUserById(int id)
        {
            ...
            string passwordDB = "";
            int typeOfPerson = 0;
            command.CommandText = "SELECT * FROM Person WHERE id = @id";//Requête SQL
            ...
            if (reader.HasRows)// Si la requête présente a des enregistrements
            {
                while (reader.Read())//Tant qu'il ya des enregistrements
                {
                    ...
                    login = long.Parse(reader["login"].ToString());//On récupère le champ login
            ...
            return new User(id, lastName, firstName, mail, phone, adress, pc, city, country, login, passwordDB, typeOfPerson);
        }
